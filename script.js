function getWeather() {
    const apiKey = 'c66b2989f4f0f7bad4120a8d54ae8d23';
...
function displayHourlyForecast(hourlyData) {
    ...
    const next24Hours = hourlyData.slice(0, 8); // Display the next 8 hours
    ...
    next24Hours.forEach(item => {
        ...
        iconSpan.classList.add('wi', `wi-owm-${weatherCode}`);
        hourlyItemDiv.appendChild(iconSpan);

        const temperatureSpan = document.createElement('span');
        temperatureSpan.textContent = `${temperature}°C`;
        hourlyItemDiv.appendChild(temperatureSpan);
